...
#[derive(Debug, Parser)]
#[command(name = "dephy-template-cli")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn init_program(args: InitProgramCliArgs) {
    ...
    let program_id = args
        .common
        .program_id
        .unwrap_or(dephy_io_dephy_template::ID);
    ...
    match client.send_and_confirm_transaction(&transaction) {
        Ok(sig) => {
            println!("Success: {:?}", sig);
            println!("Demo Created: {}", demo_account_pubkey);
        }
        Err(err) => {
            eprintln!("Error: {:?}", err);
        }
...
fn create_virtual_device(args: CreateVirtualDeviceCliArgs) {
    ...
    let demo_account_data = client.get_account_data(&demo_account_pubkey).unwrap();
    let demo_account = DemoAccount::try_from_slice(demo_account_data.as_ref()).unwrap();

    let product_metadata = {
        let product_mint_data = client.get_account_data(&demo_account.product_mint).unwrap();
