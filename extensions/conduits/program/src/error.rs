...
pub enum ConduitsError {
    ...
    NumericalOverflow,
    /// 11 - Insufficient rent
    #[error("Insufficient rent")]
    InsufficientRent,
    /// 12 - Invalid rent amount
    #[error("Invalid rent amount")]
    InvalidRentAmount,
    /// 13 - Lease not expired
    #[error("Lease not expired")]
    LeaseNotExpired,
    /// 14 - Invalid token program
    #[error("Invalid token program")]
    InvalidTokenProgram,
}

impl PrintProgramError for ConduitsError {
    fn print<E>(&self) {
        msg!(&self.to_string());
    }
}

impl From<ConduitsError> for ProgramError {
    fn from(e: ConduitsError) -> Self {
        ProgramError::Custom(e as u32)
    }
}

impl<T> DecodeError<T> for ConduitsError {
